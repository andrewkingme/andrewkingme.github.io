...
1. The code above was optimized to run as fast as possible using {% highlight text %}FilteredElementCollector(){% endhighlight %} .OfCategory(). However, looking through all sheets and views in a project is still computationally intensive. On a large benchmark project (1400+ revision clouds), the process currently takes around 4 minutes to complete.
