...
1. Element collection was optimized to run as fast as possible using `.OfCategory()`. However, filtering through all sheets and views in a project can be computationally intensive. On a large benchmark project (1400+ revision clouds), the process currently takes around 4 minutes to complete.
